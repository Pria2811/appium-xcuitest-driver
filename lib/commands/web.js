...
extensions.getExtraTranslateWebCoordsOffset = async function () {
  ...
  try {
    ...
    offset += await getSafariIsIphone(this.opts.sessionId, this) ? 10 : 0;
  } catch (err) {
...
extensions.getExtraNativeWebTapOffset = async function () {
  // let offset = await getSafariIsIphone(this.opts.sessionId, this) ?
  //   IPHONE_WEB_TAP_OFFSET :
  //   IPAD_WEB_TAP_OFFSET;
  let offset = 0;
...
extensions.translateWebCoords = async function (coords) {
  ...
  try {
    ...
    webview = await retryInterval(5, 100, async () => {
      if (await getSafariIsIphone(this.opts.sessionId, this) || (this.xcodeVersion.major === 9 && this.xcodeVersion.minor < 3)) {
        return await this.findNativeElementOrElements('-ios predicate string', `type = 'XCUIElementTypeWebView' AND visible = 1`, false);
      } else {
        // on Xcode 9.3 and ipad, the webview comes out invisible
